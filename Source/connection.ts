...
import { EventEmitter, TaskCancelledError } from "cockatiel";
...
export class Connection {
	...
	public readonly api = new Proxy({} as any, {
		get: (target, domain: string) =>
			(target[domain] = target[domain] ?? this.createDomain(domain)),
	}) as Cdp.Api;
	...
	constructor(private readonly transport: ITransport) {
		transport.onMessage((msg) =>
			this.processResponse(msg as ProtocolMessage),
		);
	}
			...
			{},
			{
				get: (_target, method: string) => {
					...
					return (params: object) =>
						this.call(`${domain}.${method}`, params);
				},
			},
		);
	}
