...
(async () => {
	...
	server.onConnection(async ([toDebugger, req]) => {
		...
		const toTarget = new Connection(
			await WebSocketTransport.create(browserInspectUri),
		);
		console.log("Connected to target");
		toTarget.onError((err) =>
			console.error("Error on target transport", err),
		);
